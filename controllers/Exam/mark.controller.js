...
export class MarkController {
  create = catchAsync(async (req, res) => {
    const data = await markService.createMark(req.tenantId, req.body, req.user?.id);
    res.status(201).json({ success: true, data });
  });

  bulkCreate = catchAsync(async (req, res) => {
    const data = await markService.bulkCreateMarks(req.tenantId, req.body.marks, req.user?.id);
    res.status(201).json({ success: true, data });
  });
  ...
  update = catchAsync(async (req, res) => {
    const data = await markService.updateMark(req.params.id, req.tenantId, req.body, req.user?.id);
    res.status(200).json({ success: true, data });
  });
