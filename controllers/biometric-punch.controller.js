...
export class BiometricPunchController extends BaseController {
  ...
  getAll = catchAsync(async (req, res) => {
    const data = await biometricPunchService.getAllPunches(
      req.tenantId,
      req.query,
    );
  ...
  getOne = catchAsync(async (req, res) => {
    const data = await biometricPunchService.getPunchById(
      req.params.id,
      req.tenantId,
    );
  ...
  update = catchAsync(async (req, res) => {
    const data = await biometricPunchService.updatePunch(
      req.params.id,
      req.tenantId,
      req.body,
    );
  ...
  delete = catchAsync(async (req, res) => {
    await biometricPunchService.deletePunch(req.params.id, req.tenantId);
    res
      .status(200)
      .json({ success: true, message: "Biometric punch deleted successfully" });
  });
}
