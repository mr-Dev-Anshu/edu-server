...
export class RoleController {
  create = catchAsync(async (req, res) => {
    const data = await roleService.createRole({ ...req.body, tenantId: req.tenantId });
  ...
  getPermissions = catchAsync(async (req, res) => {
    const data = await roleService.getPermissionsByRole(req.params.id, req.tenantId);
    res.status(200).json({ success: true, data });
  });

  update = catchAsync(async (req, res) => {
    const data = await roleService.updateRole(req.params.id, req.tenantId, req.body);
    res.status(200).json({ success: true, data });
  });
