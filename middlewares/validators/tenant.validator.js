...
const ensureBrandingAssets = (brandingAssets) => {
  ...
  for (const assetField of ["logoUrl", "faviconUrl", "coverImageUrl"]) {
    if (
      ...
      (typeof brandingAssets[assetField] !== "string" || !URL_OR_UPLOAD_PATH_REGEX.test(brandingAssets[assetField]))
    ) {
...
export const updateTenantProfileValidator = createValidator((req) => {
  ...
  ensureAtLeastOneField(
    body,
    ["name", "officialEmail", "registrationNumber", "address", "contactInfo", "settings", "customFields", "metadata"],
...
export const updateTenantBrandingValidator = createValidator((req) => {
  const { body } = req;
  ensureAtLeastOneField(body, ["themeConfig", "brandingAssets"], "themeConfig or brandingAssets is required");
  ensureThemeConfig(body.themeConfig);
  ensureBrandingAssets(body.brandingAssets);
});
...
export const listTenantsValidator = createValidator((req) => {
  ...
  if (req.query.organizationType !== undefined && !ORGANIZATION_TYPES.includes(req.query.organizationType)) {
