...
export class SendMailService {
  ...
  async sendBrevoEmail(payload) {
    ...
    const response = await axios.post(this.brevoUrl, payload, {
      headers: {
  ...
  async sendWelcomeEmail(data) {
    try {
      ...
      return await this.sendBrevoEmail(payload);
    } catch (error) {
      console.error(
      ...
      const payload = {
        ...
        htmlContent: `
          <div style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            ${
              data.testId
                ? `<p>Test ID: <strong>${data.testId}</strong></p>`
                : ""
            }
          </div>
        `,
